...
test("invalid recorder data preserves live rows and does not abort later tokens", () => {
  ...
  liveBook.usdToYes.setLevel("0.6", { price: 0.6, take: 40_380 });
  ...
  recorded.updateLevels("bid", [{ price: 0.6, shares: 100 }], 1_000);
  recorded.updateLevels("bid", [{ price: 0.6, shares: 60 }], 2_000);
  ...
  expect(state.memory("good-token")!.shellsAtPrice(0.5)).toEqual(
    recorded.shellsAtPrice(0.5),
  );
});
...
test("late recorder hydration overlays the current book on recorded history", () => {
  ...
  const recorded = new PressureFrontierMemory();
  recorded.updateLevels("bid", [{ price: 0.6, shares: 80_000 }], 1_000);
  state.hydrate({ token: recorded.snapshot() }, () => liveBook, 4_000);
  expect(state.memory("token")!.shellsAtPrice(0.5)).toEqual([
